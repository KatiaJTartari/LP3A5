package br.edu.ifsp.lp3a5.aula3.jantarFilosofos.solucao3;
 ...
 * 
 * Aqui a abordagem também é gulosa, isto é, o filósofo não larga o primeiro
 * garfo enquanto não conseguir comer, o que resolve o problema da inanição.
 * 
 * Porém, os métodos que pegam e devolvem os garfos são sincronizados,
 * garantindo que apenas um filósofo chame o método de cada vez, enquanto os
 * outros aguardam para que possam também tentar pegar os garfos da mesa.
 * 
 * Por conta disso, não haverá o deadlock de todos os filósofos aguardarem com
 * um garfo em mãos.
 * 
...
public class Mesa3 extends AbstractMesa {
	...
	/**
	 * O método sincronizado limita a que apenas uma thread execute o método de cada
	 * vez.
	 */
	public synchronized void devolveGarfos(Filosofo filosofo) {
		...
		if (this.confereStatusFilosofosAoLado(filosofo, StatusFilosofo.FOME)) {
			/*
			 * tira da espera todas as threads caso um dos filósofos ao lado esteja com
			 * FOME, isto é, tenha entrado em wait enquanto espera um garfo
			 */
			notifyAll();
		}
